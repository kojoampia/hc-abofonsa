...
import careServiceTeaserResolve from './care-service-teaser-routing-resolve.service';

describe('CareServiceTeaser routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ICareServiceTeaser returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          careServiceTeaserResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(careServiceTeaserResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);

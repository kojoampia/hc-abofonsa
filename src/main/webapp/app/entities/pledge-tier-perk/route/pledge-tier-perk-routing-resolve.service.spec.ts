...
import pledgeTierPerkResolve from './pledge-tier-perk-routing-resolve.service';

describe('PledgeTierPerk routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IPledgeTierPerk returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          pledgeTierPerkResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(pledgeTierPerkResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);

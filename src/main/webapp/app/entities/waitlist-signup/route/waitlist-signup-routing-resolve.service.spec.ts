...
import waitlistSignupResolve from './waitlist-signup-routing-resolve.service';

describe('WaitlistSignup routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IWaitlistSignup returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          waitlistSignupResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(waitlistSignupResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);

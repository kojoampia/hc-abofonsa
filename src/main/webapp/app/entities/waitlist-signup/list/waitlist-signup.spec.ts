...
import { sampleWithRequiredData } from '../waitlist-signup.test-samples';

import { WaitlistSignup } from './waitlist-signup';

vitest.useFakeTimers();

describe('WaitlistSignup Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: WaitlistSignup;
  let fixture: ComponentFixture<WaitlistSignup>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(WaitlistSignup);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 9304 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.waitlistSignups()[0]).toEqual(expect.objectContaining({ id: 9304 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 9304 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.waitlistSignups()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 9304 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to waitlistSignupService', () => {
      const entity = { id: 9304 };
      vitest.spyOn(service, 'getWaitlistSignupIdentifier');
      const id = comp.trackId(entity);
      expect(service.getWaitlistSignupIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.waitlistSignupsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.waitlistSignupsParams()).toMatchObject(

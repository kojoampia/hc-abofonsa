...
describe('MetricRollup Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: MetricRollup;
  let fixture: ComponentFixture<MetricRollup>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(MetricRollup);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 5090 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.metricRollups()[0]).toEqual(expect.objectContaining({ id: 5090 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 5090 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.metricRollups()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 5090 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to metricRollupService', () => {
      const entity = { id: 5090 };
      vitest.spyOn(service, 'getMetricRollupIdentifier');
      const id = comp.trackId(entity);
      expect(service.getMetricRollupIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.metricRollupsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.metricRollupsParams()).toMatchObject(expect.objectContaining({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] }));
  });
});

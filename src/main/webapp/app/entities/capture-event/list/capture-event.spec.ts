...
describe('CaptureEvent Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: CaptureEvent;
  let fixture: ComponentFixture<CaptureEvent>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(CaptureEvent);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 12764 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.captureEvents()[0]).toEqual(expect.objectContaining({ id: 12764 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 12764 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.captureEvents()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 12764 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to captureEventService', () => {
      const entity = { id: 12764 };
      vitest.spyOn(service, 'getCaptureEventIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCaptureEventIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.captureEventsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.captureEventsParams()).toMatchObject(expect.objectContaining({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] }));
  });
});

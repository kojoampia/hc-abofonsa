...
import pledgeTierTeaserResolve from './pledge-tier-teaser-routing-resolve.service';

describe('PledgeTierTeaser routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IPledgeTierTeaser returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          pledgeTierTeaserResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(pledgeTierTeaserResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);

...
import carePlanTeaserResolve from './care-plan-teaser-routing-resolve.service';

describe('CarePlanTeaser routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ICarePlanTeaser returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          carePlanTeaserResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(carePlanTeaserResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);

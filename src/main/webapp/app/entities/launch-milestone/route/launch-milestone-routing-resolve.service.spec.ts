...
import launchMilestoneResolve from './launch-milestone-routing-resolve.service';

describe('LaunchMilestone routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ILaunchMilestone returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          launchMilestoneResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(launchMilestoneResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);

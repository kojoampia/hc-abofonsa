...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'Oliver_Armstrong38', email: 'Roderick_Douglas9@hotmail.com' };
  ...
  describe('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('Grady_Mueller');
      cy.get(`[data-cy="login"]`).should('have.value', 'Grady_Mueller');

      cy.get(`[data-cy="firstName"]`).type('Alta');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Alta');

      cy.get(`[data-cy="lastName"]`).type('Adams');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Adams');

      cy.get(`[data-cy="email"]`).type('Hertha30@yahoo.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Hertha30@yahoo.com');

      cy.get(`[data-cy="langKey"]`).select('de');
